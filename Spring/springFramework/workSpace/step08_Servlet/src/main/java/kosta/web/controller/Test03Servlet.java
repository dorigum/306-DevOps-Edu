...
/**
 * 이 클래스가 웹브라우져에서 동작(실행) 할수 있게 하고 싶다.!
 *   => 자바중심의 코드에서 마크업을 섞어서 사용할수있다.
 * 
 * 조건 : 
 ...
 *        재정의해서 기능을 작성한다.
 *        
 *    4) 브라우져에서 어떻게 요청하면 현재 서블릿이 실행될지에해당하는 
...
public class Test03Servlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) 
		...
		//service호출 -> dao호출-> 그 결과를 받아서 처리 
		if(id==null || !id.equals("jang")) {
			//실패페이지 이동
			
			//이동전에 뷰쪽으로 전달될 데이터를 설정
			request.setAttribute("message", "id 가 없거나 또는 잘못된정보입니다.");//뷰에서 ${requestScope.message}

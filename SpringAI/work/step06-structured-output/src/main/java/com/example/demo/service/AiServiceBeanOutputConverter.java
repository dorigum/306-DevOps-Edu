...
public class AiServiceBeanOutputConverter {
  ...
  public Hotel beanOutputConverterLowLevel(String city) {
    // 구조화된 출력 변환기 생성
    BeanOutputConverter<Hotel> beanOutputConverter = new BeanOutputConverter<>(Hotel.class);//비꾸자하는 타입전달
    // 프롬프트 템플릿 생성
    ...
    log.info(beanOutputConverter.getFormat());
  ...
  public Hotel beanOutputConverterHighLevel(String city) {
    Hotel hotel = chatClient.prompt()
        ...
        .call()
        .entity(Hotel.class); // 리턴받고자 하는 객체 타입을 그냥 넣어주기만 하면 된다.
    return hotel;
  }  
}

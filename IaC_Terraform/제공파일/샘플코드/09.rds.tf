...
variable "rds_password" {
  description = "Password for the MariaDB RDS instance. Set via terraform.tfvars or TF_VAR_rds_password."
  type        = string
  sensitive   = true
}
...
resource "aws_db_instance" "terraform-mariadb-rds" {
  ...
  username             = "root"
  password             = var.rds_password # 8자 이상, 영문 대소문자, 숫자 및 특수 문자를 혼합하여 사용

...
public class FreeBoardController {
    ...
    public void list(  Model model ){
        ...
        List<FreeBoardDTO> freeList = freeBoardService.selectAll(); //N+1문제 , 페이징처리안된다.
        System.out.println("******************************");
...
//        model.addAttribute("pageList", pageList);//뷰로 전달됨(페이징처리정보 + data)
        ...
        // view에서 페이징 처리를 위한 준비
        int temp = (nowPage - 1) % BLOCK_COUNT;
        int startPage = nowPage - temp;
        model.addAttribute("startPage", startPage);

        model.addAttribute("blockCount", BLOCK_COUNT); // [1][2]..몇 개 사용
    ...
    /*
     * 글 등록 하기
     * write.jsp의 form 데이터를 받아서 수정 합니다
     * -> form의 데이터를 받을때 그냥 객체로 받아야 합니다
     */
    @PostMapping("/insert")
    ...
    public String read(@PathVariable String bno, Model model, String flag) {
        ...
        FreeBoard board = freeBoardService.selectBy(Long.parseLong(bno), state); // 조회수 증가
    ...
    public ModelAndView updateForm(String bno, Model model) {
        ...
        FreeBoard board = freeBoardService.selectBy(Long.parseLong(bno), false); // 조회 수 증가

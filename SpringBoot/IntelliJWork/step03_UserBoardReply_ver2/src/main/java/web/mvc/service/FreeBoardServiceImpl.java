...
//      return freeBoardRepository.findAll(pageable); // spring data JPA 제공하는 메소드
    ...
    public FreeBoard insert(FreeBoard board) {
        ...
        System.out.println("service: savedBoard = " + savedBoard);
        return savedBoard;
    }

    /*
     * bno로 Board 찾고 state로 조회수 올리기
     */
    @Override
    public FreeBoard selectBy(Long bno, boolean state) {
        ...
        if (state) board.setReadnum(board.getReadnum() + 1); // 변경이 감지됐기 때문에 update 실행

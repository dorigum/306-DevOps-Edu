...
public interface BoardDAO {
	...
	/*
	 * 제목에 특정 문자열이 포함된 레코드 검색 - 0개 이상의 레코드 리턴!!!
	 * select * from board where subject like ?
	 */
	List<BoardDTO> boardSelectBySubject(String keyWord) throws SearchWrongException;

	/*
	 * 글 번호에 해당하는 레코드 검색 - PK를 대상으로 조건 (레코드 수가 0, 아니면 1)
	 * select * from board where board_no = ?
	 */
	BoardDTO boardSelectByNo(int board_no) throws SearchWrongException;
	 ...
	 */
	int boardDelete(int board_no) throws DMLException;

	/*
	 * 댓글 등록하기
	 * insert into reply(reply_content, board_no, reply_regdate)
	 * values(?, ?, now());
	 */
	int replyInsert(ReplyDTO replyDTO) throws DMLException;

	/*
	 * 부모 글에 해당하는 댓글 정보 검색하기
	 * select * from board where board_no=? -> 1
	 * select * from reply where board_no=? -> n
	 * 조인 경우 : select * from board
	 * join reply using(board_no) where board_no = ?
	 */
	BoardDTO replySelectByParentNo(int board_no) throws SearchWrongException;
}
...
public class StudentTeacherDAOImpl implements StudentTeacherDAO {
	...
	/*
	 * 1번 : 성별이 여자인 학생의 정보 검색 select * from student where stu_rrn like '%-2%'
	 */
	public List<Student> getGenderByWomen() {
		...
		String sql = "select * from student where stu_rrn like '%-2%'";
	...
	/*
	 * 2. "구"를 인수로 전달 받아, 구에 거주하는 강사의 정보 검색 select * from teacher where addr like ?
	 */
	public List<Teacher> getTeacherInfoByAddr(String gu) {
		...
		try {
			...
			// ?가 있다면 setXxx() 필요하다
			ps.setString(1, "%" + gu + "%"); // 주소 like ? 인 경우
	...
	/*
	 * 5. 과목을 인수로 전달받아 그 과목을 강의하는 강사의 정보(서브쿼리) select * from teacher where Sub_code
	...
	/*
	 * 8번 먼저 뷰를 생성한다. CREATE or replace VIEW v_tear as select te.tc ,강사이름, te.수강코드,
	 * 과목, ro.강의실번호, 수용인원 from teacher te inner join subject su on te.수강코드=su.수강코드
	 * inner join sugangtb sug on su.수강코드=sug.수강코드 inner join room ro on
	 * sug.강의실번호=ro.강의실번호;
	 * 
	 * 
	 * 위의 쿼리를 실행해서 뷰로 만든다.
	 * 
	 * 그리고나서 강사의 번호를 인수로 전달받아 뷰에서 그 강사번호에 해당하는 정보를 출력한다.
	 * 
	 */

	/**
	 * 강사의 번호를 인수로 전달받아 뷰에서 그 강사번호에 해당하는 정보를 출력한다 select * from v_teacherINfo where
	 * 강사번호=?
	 * 
	 */

	public Teacher getTeacherInfoByNo(int tc_no) { // teacherNo는 PK
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;

		Teacher teacher = null;
		String sql = "select * from v_teacher where tc_no = ?";
		 ...
		 * ro on sug.강의실번호=ro.강의실번호 where te.강사번호=?";
		 */

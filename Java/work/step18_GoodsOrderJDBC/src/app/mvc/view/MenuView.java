...
public class MenuView {
	...
	public static void menu() {
		while (true) {
			...
			MenuView.printMenu(); // 1.가입 | 2.로그인 | 9.종료 메뉴를 출력하는 메소드
	...
	public static void printUserMenu(String userId) {
		while (true) {
			SessionSet ss = SessionSet.getInstance();
			System.out.println(ss.getSet()); // Set 객체
			...
			case 1:
				logout(userId);
				return; // 함수를 빠져나가서 상위 메뉴를 출력
//				 break; -> while 문만 빠져나가서 로그아웃을 하더라도 하위 메뉴가 출력됨
//					    -> 그래서 break가 아닌 return해서 상위 메뉴를 출력하게 함

			case 2:
				GoodsController.goodsSelect(); // 전체 상품 조회
				break;

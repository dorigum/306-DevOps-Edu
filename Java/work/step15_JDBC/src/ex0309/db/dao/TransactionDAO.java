...
public class TransactionDAO {
	...
	public void transfer(String inputAccount, String outputAccount, int money) {
		...
		try {
			...
			if(result == 0) throw new SQLException("출금 계좌 오류로 계좌 이제가 실패했습니다.");
			
			// 입금하기
			result = this.deposit(con, inputAccount, money);
			if(result == 0) throw new SQLException("입금 계좌 오류로 계좌 이제가 실패했습니다.");
			
			// 잔액 확인
			if(this.balanceCheck(con, inputAccount)) {
				throw new SQLException("입금 계좌 총 금액 초과로 계좌 이체가 실패했습니다.");
			}
...
//			e.printStackTrace();
	...
	// 잔액 확인
	// 입금 계좌의 총 금액이 1000원을 초과하면 true, 아니면 false를 리턴한다.
	...
	public static void main(String[] args) {
		...
		System.out.println("----1. 출금 계좌 오류----");
//		dao.transfer("A02", "A05", 200); // 입금, 출금, 금액

		System.out.println("----2. 입금 계좌 오류----");
//		dao.transfer("A04", "A01", 200); // 입금, 출금, 금액

		System.out.println("----3. 입금 계좌의 총액 1000원 이상인 경우----");

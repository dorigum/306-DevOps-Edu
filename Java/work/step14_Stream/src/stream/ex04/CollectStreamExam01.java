...
public class CollectStreamExam01 {
	...
	public static void main(String[] args) {
		...
		// ----------------------------------------------------------------------------
		// 문제) 학생들 중 남학생만 뽑아 새로운 List로 만들자
		System.out.println("1. 학생들 중 남학생만 뽑아 새로운 List로 만들자 - collect()");
		...
		// ----------------------------------------------------------------------------
		System.out.println("2. 학생들 중 남학생만 뽑아 새로운 List로 만들자 - toList()");
		List<Student> list02 = totalList.stream()
				.filter((s) -> s.getGender().equals("남"))
				.toList();
		
		// 데이터 추가
//		list01.add(new Student("미미", "남", 50)); // 추가 X(불변)
		list02.forEach(System.out::println);
		
		// ----------------------------------------------------------------------------
		System.out.println("-------toMap<K, V> 사용해보자-------");
		System.out.println("3. 학생들 정보에서 이름을 key, 점수를 value 만들어서 Map으로 리턴해보자");
